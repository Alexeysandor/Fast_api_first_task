from db import get_db, Menu, Submenu, Dish
from fastapi import APIRouter, Depends, status, HTTPException
from sqlalchemy.orm import Session
from uuid import uuid4
from validators import MenuValidator, SubmenuValidar, DishValidator

router = APIRouter(prefix='/api/v1')


@router.get('/menus')
def receive_menu(db: Session = Depends(get_db)):
    menu = db.query(Menu).all()
    return menu


@router.get('/menus/{menu_id}')
def receive_current_menu(menu_id: str, db: Session = Depends(get_db)):
...
def create_menu(menu: MenuValidator, db: Session = Depends(get_db)):
    created_menu = Menu(id=uuid4(), title=menu.title,
...
def update_current_menu(menu_id: str,
                        menu: MenuValidator,
...
def delete_current_menu(menu_id: str, db: Session = Depends(get_db)):
...
def receive_submenus(db: Session = Depends(get_db)):
    submenu = db.query(Submenu).all()
    return submenu


@router.get('/menus/{menu_id}/submenus/{submenu_id}')
def receive_current_submenus(submenu_id: str, db: Session = Depends(get_db)):
...
def create_submenu(menu_id: str, submenu: SubmenuValidar,
                   db: Session = Depends(get_db)):
    menu = db.query(Menu).filter(Menu.id == menu_id).first()
    created_submenu = Submenu(id=uuid4(), title=submenu.title,
...
def update_current_submenu(submenu_id: str, submenu: SubmenuValidar,
                           db: Session = Depends(get_db)):
...
def receive_dish(db: Session = Depends(get_db)):
...
def create_dish(menu_id: str, submenu_id: str, dish: DishValidator,
    ...
    created_dish = Dish(id=uuid4(), title=dish.title,
                        description=dish.description,
                        price=dish.price, submenu_id=submenu_id)
    db.add(created_dish)
    db.commit()
...
def delete_current_dish(menu_id: str, submenu_id: str, dish_id: str,
    ...
    dish_to_delete = db.query(Dish).filter(Dish.id == dish_id).first()
